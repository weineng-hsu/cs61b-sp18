...
public class World {
    ...
    private class Room {
        ...
        public Room(Position BL, int wide, int height) {
            bottomLeft = BL;
            roomWide = wide;
            roomHeight = height;
            upperRight = new Position(BL.x + wide - 1, BL.y + height - 1);
        }
        public Room(int wide, int height, Position UR) {
            upperRight = UR;
            roomWide = wide;
            roomHeight = height;
    ...
    public World(int s, int h, int w) {
        HEIGHT = h;
        WIDTH = w;
        r = new Random(s);
        initilize();
    ...
    public void generateRoomExit(Room room) {
        for (int i = 0; i < 7; i++) {
            ...
            switch (randomDir) {
                case EAST:
                    Position exitEast = new Position(room.upperRight.x, room.upperRight.y - 1 - RandomUtils.uniform(r, room.roomHeight - 2));
                    if (grid[exitEast.x][exitEast.y] == Tileset.WALL) {
                        room.exitEast = exitEast;
                    }
                    break;
                case SOUTH:
                    Position exitSouth = new Position(room.bottomLeft.x + 1 + RandomUtils.uniform(r, room.roomWide - 2), room.bottomLeft.y);
                    if (grid[exitSouth.x][exitSouth.y] == Tileset.WALL) {
                        room.exitSouth = exitSouth;
                    }
                    break;
                case WEST:
                    Position exitWest = new Position(room.bottomLeft.x, room.upperRight.y - 1 - RandomUtils.uniform(r, room.roomHeight - 2));
                    if (grid[exitWest.x][exitWest.y] == Tileset.WALL) {
                        room.exitWest = exitWest;
                    }
                    break;
                case NORTH:
                    Position exitNorth = new Position(room.bottomLeft.x + 1 + RandomUtils.uniform(r, room.roomWide - 2), room.upperRight.y);
    ...
    public void drawNextRoom(Room origin) {
        if (origin.exitEast != null) {
            ...
            int nextRoomBottomY = RandomUtils.uniform(r, origin.exitEast.y + 2 - nextRoomHeight, origin.exitEast.y);
        ...
        if (origin.exitSouth != null) {
            int nextRoomHeight = RandomUtils.uniform(r, 5) + 3;
            int nextRoomWeith = RandomUtils.uniform(r, 5) + 3;
            int nextRoomUpperX = RandomUtils.uniform(r, origin.exitSouth.x + 1, origin.exitSouth.x - 1 + nextRoomWeith);
        ...
        if (origin.exitWest != null) {
            int nextRoomHeight = RandomUtils.uniform(r, 5) + 3;
            int nextRoomWeith = RandomUtils.uniform(r, 5) + 3;
            int nextRoomUpperY = RandomUtils.uniform(r, origin.exitWest.y + 1, origin.exitWest.y - 1 + nextRoomHeight);
        ...
        if (origin.exitNorth != null) {
            int nextRoomHeight = RandomUtils.uniform(r, 5) + 3;
            int nextRoomWeith = RandomUtils.uniform(r, 5) + 3;
            int nextRoomBottomX = RandomUtils.uniform(r, origin.exitNorth.x + 2 - nextRoomWeith, origin.exitNorth.x);
